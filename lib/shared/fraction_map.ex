defmodule Shared.FractionMap do
  ...
  The map is padded to include all letters of the Configuration.alphabet().
  """
  @spec calculate_fraction(count_map(), integer()) :: t()
  def calculate_fraction(frequency_map, total_count) do
    frac_map = Map.new(frequency_map, fn {key, value} -> {key, value / total_count} end)

    # Pad map with not already existing letters.
    Configuration.alphabet()
    |> String.graphemes()
    |> Enum.reduce(frac_map, fn letter, map -> Map.put_new(map, letter, 0) end)
  end
end
